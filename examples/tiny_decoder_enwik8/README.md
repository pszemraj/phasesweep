...
The config uses 1000 training batches per trial. The upstream trainer keeps its `validate_every: 100` periodic cadence at steps 0-900, then saves `final.pt` at step 1000. After training exits, `run_trial.py` reloads that checkpoint and runs one additional validation with the same validation split, sequence length, batch size, `val_batches`, device, autocast settings, and fixed seed. Only this step-1000 `final_checkpoint` evaluation is published as the PhaseSweep objective. The atomic, versioned `result.json` retains top-level `val_loss` and also records completion status, generation and attempt IDs, objective name/split/value, checkpoint policy/name/step, resolved overrides, and a SHA-256 of the exact overrides JSON bytes supplied to the wrapper. The configured `json_envelope` extractor verifies those identities, the overrides digest, and the declared final-checkpoint policy before accepting the value. The upstream template does not currently expose warmup ratio or grouped-query attention controls, so this example sticks to trainer hyperparameters it supports. One more upstream quirk to know before sweeping shape keys: the SwiGLU feedforward rounds its hidden width up to a multiple of 256, so at `dim: 128` every `ffn_dim_multiplier` value up to 2.0 builds the same 256-wide FFN and sweeping the knob changes nothing until the requested width crosses a rounding step.
