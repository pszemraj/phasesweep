...
``install`` pins the absolute ``phasesweep-mcp`` executable from the active
environment. :func:`~phasesweep.mcp.install.installer.check_install` is the
read-only counterpart: it verifies each configured client's launcher still
resolves and never edits a client file.
"""
