...
9. Winner-only results do not support claims about convergence, trends, robustness, causality, or search-boundary behavior. Report the returned `represented_generation_id` and result context; historical metric names, goals, and phase labels belong to that generation, not to a later catalog edit. Use the returned objective assurance: W&B is keyed by attempt ID, JSON/log sources are local, and only envelopes validate evaluation metadata. Frozen reads require no remote access.
