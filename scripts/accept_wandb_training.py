...
def main(argv: list[str] | None = None) -> int:
    """Run the explicitly approved four-attempt CPU acceptance command."""
    ...
    experiment = Experiment(
        experiment="wandb_linear_acceptance",
        workdir=str(workdir),
        storage="auto",
        provenance={"trainer": "wandb-linear-20-steps-v1", "data": "fixed-linear-64-32-v1"},
    ...
    print(
        f"Launch budget: four sequential CPU attempts at {extractor.base_url}/"
        f"{args.entity}/{args.project}; 20 full-batch steps each; "
        "64 training / 32 held-out examples; trainer cap=120s, polling cap=120s, "
        "phase cap=600s, experiment cap=1200s; cleanup grace is separate.",
        flush=True,
    )
    ...
    for phase in experiment.phases:
        study = optuna.load_study(
            study_name=f"{experiment.experiment}::{phase.name}", storage=experiment.resolved_storage
        )
        trials = study.trials
        assert len(trials) == 2
