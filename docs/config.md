...
The two are bound to each other. `workdir` stays outside every semantic fingerprint - a result tree is meant to be movable - so on persistent storage each phase study instead records the one resolved `<workdir>/<experiment>` root it publishes into, and an invocation that offers a different root is refused before any reaping, top-up, or publication. Without that binding one study could back two divergent result trees, each claiming to be its publication while its trial artifacts lived under the other. The claim happens on first contact, which also adopts studies created before the binding existed: the first invocation to see such a study binds it to *its own* root. Changing `workdir` afterwards is therefore an explicit two-step operation - move or copy the artifact tree yourself, then run `phasesweep rebind-workdir CONFIG` with the config that already names the new location. The command takes the same locks a run does, refuses unless the destination holds the experiment's namespace and (when storage records completed trials) a last-success pointer whose generation manifest validates there, and writes nothing when it refuses. It rebinds only: it never copies, deletes, or checks the tree you moved away from. An in-memory study binds nothing, since it cannot outlive the process that created it.
