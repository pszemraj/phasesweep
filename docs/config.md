...
Agent-visible metric descriptors label the configured extractor's evidence assurance. `json_envelope` is attempt-, checkpoint-, and evaluation-policy-bound. `log_regex` and W&B are attempt-bound by the unique trial directory or immutable run ID, but are labeled as not checkpoint- or policy-bound.
