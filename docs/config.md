...
The two are bound to each other. `workdir` stays outside every semantic fingerprint - a result tree is meant to be movable - so on persistent storage each phase study instead records the one resolved `<workdir>/<experiment>` root it publishes into, and an invocation that offers a different root is refused before any reaping, top-up, or publication. Without that binding one study could back two divergent result trees, each claiming to be its publication while its trial artifacts lived under the other. The claim happens on first contact, but only for a study that holds no trials; a populated study created before the binding existed is refused instead of adopted, because which workdir owns its evidence cannot be inferred. Changing `workdir` afterwards is therefore an explicit two-step operation - move the artifact tree yourself, then run `phasesweep rebind-workdir CONFIG` with the config that already names the new location (that command is also how a pre-binding study is migrated, pointed at its original tree). The command takes the same locks a run does, writes nothing when it refuses, and refuses unless the destination holds the experiment's namespace, the evidence directory of every trial in the ledger, no RUNNING trial and no unresolved attempt, and (when storage records completed trials) a last-success pointer whose generation manifest validates there; a suite that may have published a suite generation is refused outright. It rebinds only: it never copies, deletes, or checks the tree you moved away from. An in-memory study binds nothing, since it cannot outlive the process that created it.
...
Because those values are unfingerprinted, PhaseSweep records *which* environment each trial ran under: every trial stores the SHA-256 digest of its composed trainer environment and the sorted list of variable names it contained, and the winner stores that digest plus the `inherit_env` contract. Values are never stored by default - the digest and names are what make two runs comparable without exposing a token. `execution.record_env: true` additionally writes the full mapping to `environment.json` in each trial directory, owner-only (0600), when you need the raw values for an audit. The digest deliberately stays out of every fingerprint, so an environment change never invalidates a study or blocks a top-up; instead PhaseSweep warns when the trials being compared in one phase span more than one environment, when a `--from-phase` resume inherits a winner produced under a different environment, and, during preflight, when `inherit_env: all` is combined with persistent storage. See [runtime behavior](runtime.md#output-layout) for how the record is stored.
