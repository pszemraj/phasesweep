...
While confirmed recovery holds a run transition lock, status and launch-capacity checks keep an unresolved dead run counted as running without waiting for evidence reconciliation, while status and await still report that recovery is required. They reevaluate cleanup once recovery releases the lock.
...
Cleanup confirmation comes from the engine shutdown handler after it terminates active trial process groups through the stale-recovery cleanup path. If a spawned runner disappears without terminal status, cancellation cannot observe status, or shutdown reports unconfirmed trial cleanup, the server writes a marker that keeps the run live and counted against [`max_concurrent_runs`](#concurrency-and-single-gpu-hosts). Timeout, stale sweeping, and retry do not clear it, so a runner killed by SIGKILL or the OOM killer requires operator recovery. A recorded boot ID from an earlier boot proves the runner and descendants are gone and prevents signals from targeting reused process IDs. A no-status run can therefore fail and release its concurrency slot without process recovery. Terminal `cleanup_confirmed: false` still requires operator recovery until durable trial rows and its frozen result are reconciled, but it does not keep a prior-boot process slot occupied. Older handles and hosts without a boot ID retain the process recovery requirement. Normal shutdown tears down trial groups, and the stale reaper handles uncertain trainer leftovers before later launches.
