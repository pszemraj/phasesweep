...
Use `recovery_required` as the decision field: when true, stop monitoring and report the uncertain launch, cleanup, or interrupted result finalization to the user. `await_run` returns immediately with `reason: recovery_required`; status and latest-run payloads expose the same flag in their run metadata. After successful operator recovery, this flag becomes false while the terminal `failure`, including its original `remediation`, remains frozen as history; do not repeat recovery based on that historical text. A launch handoff observed mid-transition can still settle when the runner finishes registering itself, but cleanup uncertainty and interrupted finalization require operator action.
