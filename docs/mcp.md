...
| `await_run` | `run_id`, optional `timeout_seconds` (5-600; default 120) | read (waits) | the `get_run_status` payload plus `changed` and `reason` (`recovery_required` / `terminal` / `phase_completed` / `timeout`) |
