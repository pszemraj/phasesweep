...
| `get_run_results` | exactly one of `experiment_id` or `run_id` | read | objective metadata, result provenance, declared/winner counts, missing phases, all-phases completeness, safe terminal failure context, and per-winner source, generation, promotion context, trial number, metric, policy-filtered sampled params, gate status, and partial-winner status |
...
Every structured tool result also carries `next_action`, containing the normal next tool name when another automatic workflow step is safe and `null` when the workflow is complete or user/operator input is required.
