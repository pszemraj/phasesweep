...
Every top-up attaches the sampler declared by the phase rather than the Optuna default. Seeded random sampling derives each draw from the durable study name, trial number, and parameter name, so one uninterrupted invocation and arbitrarily batched top-ups produce the same parameter sequence. Grid sampling resumes from stored grid assignments. TPE and CMA-ES keep process-local continuation state that Optuna does not fully persist, so PhaseSweep rejects their cross-invocation top-ups before mutation; use a new experiment name or run the full target in one invocation.
