...
Each trainer starts behind a supervisor ready/ack handshake. The supervisor cannot exec the trainer until PhaseSweep atomically writes and fsyncs `process_identity.json` with the attempt ID, PID, PGID, Linux process start time, and host boot ID. If the orchestrator dies before acknowledgement, pipe EOF makes the supervisor exit without starting training. The identity record is removed on clean exit and preserved on failure for inspection. If identity persistence fails, PhaseSweep terminates the blocked supervisor group before returning a failed trial result.
