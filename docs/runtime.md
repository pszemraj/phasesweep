...
SIGTERM, SIGINT, and SIGHUP trigger shutdown cleanup. The handler sends SIGTERM to active groups, waits briefly, sends SIGKILL to survivors, then does one bounded, unconditional reap of the direct child before reporting cleanup as confirmed, and exits with `128 + signum`. The bounded reap closes a race where a single nonblocking poll could see "not yet exited" right before the child died, leaving a zombie despite cleanup being reported confirmed. SIGKILL and hard OOM kills cannot be caught by Python.

Cleanup is bounded but not immediate. A trial group that ignores SIGTERM costs the full 10-second grace, then roughly 2 seconds to confirm the SIGKILL, then up to 5 more for the bounded direct-child reap: about 17 seconds worst case. A shutdown signal that arrives while a trial launch is still in flight is deferred until that launch finishes its own cleanup, so an interactive Ctrl-C can take that long to return the prompt. It is not a hang; sending a second Ctrl-C only risks leaving the group behind.
