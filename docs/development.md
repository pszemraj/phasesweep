...
Run `pytest` by itself, with no concurrent lint, type-check, or build jobs. Some process-supervision and timeout tests are timing-sensitive and can fail under unrelated validation load. A clean full-suite run should not print a warning summary; investigate and fix new warnings instead of accepting them as background noise. The Linux CI job runs these commands in sequence with Python 3.13, Optuna 4.9.0, and the declared development dependencies. Hardware tests remain excluded by default. This initial acceptance environment does not establish the full Python, Optuna, or platform support matrix.
