...
- TODO(mcp): Add an aggregated read-only trial-count path for non-SQLite Optuna storage before recommending very frequent `phasesweep_get_status` polling on very large RDB-backed studies; the current generic Optuna load path is acceptable for normal agent polling cadences, but high-frequency polling over large external studies should have backend-specific query coverage.
...
- TODO(example): Update the `examples/tiny_decoder_enwik8/upstream` submodule after the trainer template handles CPU autocast as fp32/disabled by default, uses CUDA-only `pin_memory`, and moves batches with CUDA-only `non_blocking=True`; keep those PyTorch training changes in the upstream trainer repo rather than patching the gitlink contents here.
